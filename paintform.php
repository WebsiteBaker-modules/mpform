    ...
    function make_checkbox(
        ...
        if (in_array($v, $value) or ($isnew and $def > 0)) {
            $option = '<input '
               ...
               . ' checked="checked" />'
               . '<label for="'.$label_id.'"'
               . 'class="'.MPFORM_CLASS_PREFIX.'checkbox_label">'
               . $vals[1]
...
if (!function_exists('make_radio')) {
    function make_radio(
        ...
        if (($v == $value) or ($isnew and $def > 0)) {
            ...
            . '<label'
            . ' for="'.$label_id.'"'
            . ' class="'.MPFORM_CLASS_PREFIX.'radio_label">'
            . $vals[1]
            . '</label>'.$seperator.PHP_EOL; 
        } else {
            $option = '<input'
